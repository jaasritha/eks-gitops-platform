################################################################################
# Staging Environment — Terraform root module
################################################################################
...
terraform {
  ...
  backend "s3" {
    bucket         = "eks-gitops-tfstate-staging"
...
module "vpc" {
  ...
  single_nat_gateway = true   # staging: single NAT to save cost, but prod-like config otherwise
  tags               = local.common_tags
}
...
module "eks" {
  ...
  node_groups = {
    general = {
      ...
      capacity_type  = "ON_DEMAND"   # staging uses ON_DEMAND for stability (prod parity)
      desired_size   = 2
      min_size       = 2
      max_size       = 8
      labels         = { role = "general" }
